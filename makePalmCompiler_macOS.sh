...
cd $DIR/output
../prc-tools-2.3/configure --enable-languages=c,c++ --enable-targets=m68k-palmos,arm-palmos --disable-nls --disable-doc --with-palmdev-prefix=$DIR/palmSdk --host=i686-apple-darwin || exit
make || exit
...
sudo cp ./libpisock/.libs/libpisock.9.0.0.dylib /usr/local/lib || exit
sudo cp ./libpisock/.libs/libpisock.9.dylib /usr/local/lib || exit
sudo cp ./libpisock/.libs/libpisock.dylib /usr/local/lib || exit
